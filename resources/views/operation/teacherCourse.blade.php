...
<html>
	...
	<body>
		<div class="col-12 p-2">
			<form name="insertTeacherCourse" method="POST" action="{{ url('/operation/insertTeacherCourse') }}" enctype="multipart/form-data">
				{{ csrf_field() }}
				<div class="form-group row">
					<label for="courseid" class="col-sm-2 col-form-label">教师</label>
					<div class="col-sm-10">
						<select name="id" class="selectpicker form-control" id="id" data-live-search="true" data-max-options="1">
							@for ($i = 0; $i < $teacherCourse[0]['row_num2']; $i++)
								<option value="{{ $teacherCourse[0]['row2'][$i]['id'] }}">{{ $teacherCourse[0]['row2'][$i]['id'] }} : {{ $teacherCourse[0]['row2'][$i]['username'] }}</option>
							@endfor
						</select>
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<select name="courseid" class="selectpicker form-control" id="courseid" data-live-search="true" data-max-options="1">
							@for ($i = 0; $i < $teacherCourse[1]['row_num1']; $i++)
								<option value="{{ $teacherCourse[1]['row1'][$i]['courseid'] }}">{{ $teacherCourse[1]['row1'][$i]['courseid'] }} : {{ $teacherCourse[1]['row1'][$i]['coursename'] }}</option>
							@endfor
						</select>
					</div>
				</div>
				<div class="form-group row">
					<div class="col-sm-10">
						<button name="insertTeacherCourse" type="submit" class="btn btn-primary">授课</button>
					</div>
				</div>
			</form>
		</div>
